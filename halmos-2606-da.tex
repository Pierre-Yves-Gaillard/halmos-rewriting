...
Here is a rewriting of some proofs from Halmos's wonderful book titled \textbf{Naive Set Theory} \cite{halmos_naive_1974}. \footnote{This text is available at \url{https://github.com/Pierre-Yves-Gaillard/halmos-rewriting}} Here is a more precise formulation: 
More precisely, we are just rewriting Section~12 titled \textit{The Peano Axioms} and the beginning of Section~13 titled \textit{Arithmetic}. 
...
\end{lem} 
\begin{proof} 
We prove \(i\in j\in k\implies i\in k\) by induction on \(k\). Denote by \(A(k)\) the statement to be proved. Then \(A(0)\) is obvious. To prove \(A(k)\implies A(k^+)\), we assume \(A(k)\) and \(i\in j\in k^+\), and we must show \(i\in k^+\). By \eqref{Ej+}, we have \(j\in k\) or \(j=k\), and we have to prove \(i\in k\) or \(i=k\). We easily see that we have in fact \(i\in k\) in both cases. 
\end{proof} 

\begin{proof}[Proof of \eqref{E4}] 
Assume \(i\ne j\) for the sake of contradiction. We get \(i\in j\in i\) by \eqref{Ej+}, hence \(i\subset j\subset i\) by Lemma~\ref{Ltrans} (implication \(m\in n\implies m\subset n\)), hence \(i=j\). 
\end{proof} 
...
\end{lem} 
\begin{proof} 
We first show that for all \(i,j\) we have \(i\in j\implies i^+\in j^+\). Let us denote the statement 
\[
i\in j\implies i^+\in j^+.
\] 
by \(B(j)\) (here \(i\) is considered as a parameter). The base case \(B(0)\) is vacuously true since \(i\in 0\) is false. Thus, it is enough to verify \(B(j)\implies B(j^+)\). So we assume \(B(j)\) and \(i\in j^+\), and we must show \(i^+\in j^{++}\). In view of \eqref{Ej+}, this means that we must prove 
\[
B(j)\land(i\in j\ \lor\ i=j)\implies i^+\in j^+\ \lor\ i^+=j^+,
\] 
which is trivial. 

We now prove that for all \(i,j\) we have \(i^+\in j^+\implies i\in j\), that is 
\[
i^+\in j\ \lor\ i^+=j\implies i\in j.
\] 
This follows from \eqref{Ej+} and Lemma~\ref{Ltrans}. This completes the proof. 
\end{proof} 
...
Theorem \ref{Torder} follows from Lemmas \ref{Ltrans} and \ref{Ltricho}. 
\end{proof} 
